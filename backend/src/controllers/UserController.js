...
class UserController {
    ...
    // 3. Đặt lại mật khẩu đăng nhập (Quên mật khẩu)
    resetPassword = async (req, res) => {
        try {
            const { phone, newPassword } = req.body;
            if (!phone || !newPassword) {
                return res.status(400).json({ success: false, message: "Vui lòng nhập đủ số điện thoại và mật khẩu mới" });
            }
    ...
    // 4. Lấy thông tin cá nhân (FIX LỖI MÃ PIN)
    getProfile = async (req, res) => {
        try {
            // Thêm +paymentPassword để kiểm tra sự tồn tại thực tế của PIN
            ...
            // Logic quan trọng: Kiểm tra xem trường paymentPassword có dữ liệu hay không
            // để ghi đè lại trường hasPaymentPin cho Frontend
            userData.hasPaymentPin = !!(user.paymentPassword && user.paymentPassword !== "");

            // Xóa mã hash paymentPassword trước khi trả về client
            delete userData.paymentPassword;
    ...
    // 5. Lấy thông tin trang cá nhân công khai
    ...
    // 8. Lấy danh sách Followers & Following
    ...
    updateProfileImage = async (req, res) => {
        try {
            ...
            const updateData = {};
            if (req.file.fieldname === 'avatar') updateData.avatar = imageUrl;
            else if (req.file.fieldname === 'cover') updateData.cover = imageUrl;

            const updatedUser = await User.findByIdAndUpdate(userId, updateData, { new: true }).select('-password');
            res.status(200).json({ success: true, message: "Cập nhật ảnh thành công!", data: updatedUser });
        } catch (error) {
            res.status(500).json({ success: false, message: "Lỗi upload: " + error.message });
        }
    }

    // 12. Thiết lập mã PIN lần đầu
    setupPaymentPassword = async (req, res) => {
        try {
            ...
            if (user.paymentPassword) return res.status(400).json({ success: false, message: "Mã PIN đã tồn tại!" });

            const isMatch = await UserService.comparePassword(password, user.password);
            if (!isMatch) return res.status(400).json({ success: false, message: "Mật khẩu đăng nhập không chính xác" });

            if (!/^\d{6}$/.test(pin)) return res.status(400).json({ success: false, message: "Mã PIN phải bao gồm 6 chữ số" });

            const salt = await UserService.generateSalt(10);
            const hashedPin = await UserService.hashPassword(pin, salt);

            await User.findByIdAndUpdate(userId, { paymentPassword: hashedPin, hasPaymentPin: true });
    ...
    // 13. Đặt lại mã PIN qua OTP
    resetPaymentPinByOTP = async (req, res) => {
        try {
            const { newPin } = req.body;
            const userId = req.user.id;
            if (!/^\d{6}$/.test(newPin)) return res.status(400).json({ success: false, message: "Mã PIN mới phải bao gồm 6 chữ số" });
